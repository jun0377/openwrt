...
define DownloadMethod/unknown
	echo "[download.mk] ERROR: No download method available"; false
endef
...
define DownloadMethod/cvs
	$(call wrap_mirror,$(1),$(2), \
		echo "[download.mk] Checking out files from the cvs repository..."; \
		mkdir -p $(TMP_DIR)/dl && \
		cd $(TMP_DIR)/dl && \
		rm -rf $(SUBDIR) && \
		[ \! -d $(SUBDIR) ] && \
		cvs -d $(URL) export $(SOURCE_VERSION) $(SUBDIR) && \
		echo "[download.mk] Packing checkout..." && \
...
define DownloadMethod/svn
	$(call wrap_mirror,$(1),$(2), \
		echo "[download.mk] Checking out files from the svn repository..."; \
		mkdir -p $(TMP_DIR)/dl && \
		cd $(TMP_DIR)/dl && \
		rm -rf $(SUBDIR) && \
		[ \! -d $(SUBDIR) ] && \
...
define DownloadMethod/rawgit
	$(call wrap_mirror,$(1),$(2), \
		echo "[download.mk] Checking file is exist..."; \
		if [ -f ${DL_DIR}/${FILE} ]; then \
			echo "[download.mk] ${FILE} exist! return now"; \
		else \
			echo "[download.mk] Checking out files from the git repository..."; \
			echo "[download.mk] mkdir -p $(TMP_DIR)/dl " && \
			mkdir -p $(TMP_DIR)/dl && \
			echo "[download.mk] cd $(TMP_DIR)/dl" && \
			cd $(TMP_DIR)/dl && \
			echo "[download.mk] rm -rf $(SUBDIR)" && \
			rm -rf $(SUBDIR) && \
			[ \! -d $(SUBDIR) ] && \
			echo "[download.mk] git clone $(OPTS) $(URL) $(SUBDIR)" && \
			git clone $(OPTS) $(URL) $(SUBDIR) && \
			echo "[download.mk] cd $(SUBDIR) && git checkout $(SOURCE_VERSION)" && \
			(cd $(SUBDIR) && git checkout $(SOURCE_VERSION)) && \
			export TAR_TIMESTAMP=$(cd $(SUBDIR) && git log -1 --format="@%ct") && \
			echo "[download.mk] Generating formal git archive (apply .gitattributes rules)" && \
...
define DownloadMethod/bzr
	$(call wrap_mirror,$(1),$(2), \
		echo "[download.mk] Checking out files from the bzr repository..."; \
		mkdir -p $(TMP_DIR)/dl && \
		cd $(TMP_DIR)/dl && \
		rm -rf $(SUBDIR) && \
		[ \! -d $(SUBDIR) ] && \
...
define DownloadMethod/hg
	$(call wrap_mirror,$(1),$(2), \
		echo "[download.mk] Checking out files from the hg repository..."; \
		mkdir -p $(TMP_DIR)/dl && \
		cd $(TMP_DIR)/dl && \
		rm -rf $(SUBDIR) && \
		[ \! -d $(SUBDIR) ] && \
...
define DownloadMethod/darcs
	$(call wrap_mirror, $(1), $(2), \
		echo "[download.mk] Checking out files from the darcs repository..."; \
		mkdir -p $(TMP_DIR)/dl && \
		cd $(TMP_DIR)/dl && \
		rm -rf $(SUBDIR) && \
		[ \! -d $(SUBDIR) ] && \
