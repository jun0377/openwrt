...
class DownloadGitHubTarball(object):
    ...
    def download(self):
        ...
        sys.stderr.write('[dl_github_archive.py] download...\n')

        # 检查目标文件是否已经存在，避免重复下载
        target_file = os.path.join(self.dl_dir, self.source)
        sys.stderr.write('[dl_github_archive.py] target_file: {} \n' .format(target_file))
        if os.path.exists(target_file):
            try:
                self._hash_check(target_file)
                return
            except Exception:
                pass
    ...
    def _init_commit_ts(self):
        ...
        # 检查版本字符串是否为完整的SHA1哈希值，SHA1哈希值的长度固定为40个字符
    ...
    def _error(self, msg):
        return DownloadGitHubError('[dl_github_archive.py] {}: {}'.format(self.source, msg))
...
def main():
    ...
    sys.stderr.write('[dl_github_archive.py] {}: Download from {} ...\n'.format(args.source, args.url))

    try:
        method = DownloadGitHubTarball(args)
        method.download()
    except Exception as ex:
        sys.stderr.write('[dl_github_archive.py] {}: Download from {} failed\n'.format(args.source, args.url))
        sys.stderr.write('[dl_github_archive.py]{}\n'.format(ex))
        sys.exit(1)
